...
void Map::Attack(Character *from, Direction direction)
{
	...
	// Mining system - check for mining after NPC attacks (when no NPCs are at target location)
	// Reuse existing target_x and target_y variables, but reset them to player position first
	target_x = from->x;
	target_y = from->y;
	...
	// Check for mining on Unknown Tile 3 (TileSpec 12)
	Map_Tile::TileSpec tile_spec = this->GetSpec(target_x, target_y);

	if (tile_spec == Map_Tile::Unknown3)
	{
		// Attempt mining
		from->AttemptMining(target_x, target_y);
		return;
	}
}
...
bool Map::Walkable(unsigned char x, unsigned char y, bool npc) const
{
	...
	const Map_Tile& tile = this->GetTile(x, y);
	bool tile_walkable = tile.Walkable(npc);

	// DEBUG: Log walkability check for Unknown2 tiles
	if (tile.tilespec == Map_Tile::Unknown2)
	{
		// We can't use ServerMsg here, but we can check the logic
		// Unknown2 should return false from tile.Walkable(npc)
	}

	if (!tile_walkable)
		return false;

	if (this->world->config["GhostArena"] && tile.tilespec == Map_Tile::Arena && this->Occupied(x, y, PlayerAndNPC))
		return false;
