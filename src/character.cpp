	...
	, display_cha(this->world->config["UseAdjustedStats"] ? adj_cha : cha)
{
	...
	Database_Result res = this->world->db->Query("SELECT `name`, `title`, `home`, `fiance`, `partner`, `admin`, `class`, `gender`, `race`, `hairstyle`, `haircolor`,"
		...
		"`karma`, `sitting`, `hidden`, `bankmax`, `goldbank`, `usage`, `crafting_level`, `crafting_exp`, `crafting_exp_tnl`, `mining_level`, `mining_exp`, `mining_exp_tnl`, `baglevel`, `inventory`, `bank`, `bag`, `paperdoll`, `spells`, `guild`, `guild_rank`, `guild_rank_string`, `quest`, `vars`, "
	...
	this->mining_level = GetRow<int>(row, "mining_level");
	this->mining_exp = GetRow<int>(row, "mining_exp");
	this->mining_exp_tnl = GetRow<int>(row, "mining_exp_tnl");
	...
	if (this->baglevel < 1) this->baglevel = 1; // Safety check
...
void Character::Refresh()
{
	...
	builder.AddChar(static_cast<char>(updatecharacters.size())); // Number of players
	builder.AddByte(255);
...
void Character::ShowBoard(Board *board)
{
	...
	builder.AddChar(static_cast<unsigned char>(board->posts.size()));
...
int Character::Usage()
{
	return this->usage + (static_cast<int>(std::time(0)) - this->login_time) / 60;
}
...
void Character::SpikeDamage(int amount)
{
	...
	builder3.AddChar(static_cast<unsigned char>(util::clamp<int>(static_cast<int>(double(this->hp) / double(this->maxhp) * 100.0), 0, 100)));
...
{
	this->muted_until = static_cast<int>(time(0)) + int(this->world->config["MuteLength"]);
...
void Character::Save()
{
	...
	this->world->db->Query("UPDATE `characters` SET `title` = '$', `home` = '$', `fiance` = '$', `partner` = '$', `admin` = #, `class` = #, `gender` = #, `race` = #, "
		...
		"`nointeract` = #, `bankmax` = #, `goldbank` = #, `usage` = #, `crafting_level` = #, `crafting_exp` = #, `crafting_exp_tnl` = #, `mining_level` = #, `mining_exp` = #, `mining_exp_tnl` = #, `baglevel` = #, `inventory` = '$', `bank` = '$', `bag` = '$', `paperdoll` = '$', "
		...
		nointeract, this->bankmax, this->goldbank, this->Usage(), this->crafting_level, this->crafting_exp, this->crafting_exp_tnl, this->mining_level, this->mining_exp, this->mining_exp_tnl, this->baglevel, ItemSerialize(this->inventory).c_str(), ItemSerialize(this->bank).c_str(), ItemSerialize(this->bag).c_str(),
...
void Character::GiveMiningEXP(int amount)
{
	if (amount <= 0) return;

	this->mining_exp += amount;

	// Level up logic - 100 exp per level + 50 exp bonus per level
	while (this->mining_level < 100 && this->mining_exp >= this->mining_exp_tnl)
	{
		this->mining_exp -= this->mining_exp_tnl;
		this->mining_level++;

		// Calculate next level requirement: 100 + (level * 50)
		this->mining_exp_tnl = 100 + (this->mining_level * 50);

		// Send level up message
		PacketBuilder reply(PACKET_RECOVER, PACKET_REPLY, 7);
		reply.AddInt(this->mining_exp);
		reply.AddShort(this->mining_level);
		reply.AddChar(0); // Stat points (not used for mining)
		this->Send(reply);

		this->ServerMsg(this->world->i18n.Format("mining_level_up", this->mining_level));
	}
}

bool Character::IsMiningWeapon(int weapon_id)
{
	// Check if the weapon is defined as a mining weapon in mining.ini
	auto mining_weapons = this->world->mining_config.equal_range("MiningWeapon");

	for (auto it = mining_weapons.first; it != mining_weapons.second; ++it)
	{
		int config_weapon_id = static_cast<int>(it->second);

		if (config_weapon_id == weapon_id)
		{
			return true;
		}
	}
...
bool Character::AttemptMining(unsigned char target_x, unsigned char target_y)
{
	// Check if target tile is Unknown Tile 3 (mining tile)
	Map_Tile::TileSpec tile_spec = this->map->GetSpec(target_x, target_y);

	if (tile_spec != Map_Tile::Unknown3)
	{
		return false;
	}

	// Check if player has a mining weapon equipped
	int weapon_id = this->paperdoll[Character::Weapon];

	if (weapon_id == 0 || !IsMiningWeapon(weapon_id))
	{
		return false;
	}

	// Get all possible mining items and their configurations
	std::vector<int> possible_items;
	for (const auto& config_pair : this->world->mining_config)
	{
		const std::string& key = config_pair.first;

		// Look for items with chance configuration (format: "itemid.chance")
		size_t dot_pos = key.find(".chance");
		if (dot_pos != std::string::npos)
		{
			int item_id = util::to_int(key.substr(0, dot_pos));

			// Check level requirement
			std::string level_key = util::to_string(item_id) + ".level_req";
			auto level_req_setting = this->world->mining_config.find(level_key);
			int required_level = 1;
			if (level_req_setting != this->world->mining_config.end())
			{
				required_level = static_cast<int>(level_req_setting->second);
			}

			// Only add items that the player can mine
			if (this->mining_level >= required_level)
			{
				possible_items.push_back(item_id);
			}
		}
	}

	// If no items available for this level, use default chance
	if (possible_items.empty())
	{
		return false;
	}

	// Try to mine each possible item based on chance
	for (int item_id : possible_items)
	{
		std::string chance_key = util::to_string(item_id) + ".chance";
		auto chance_setting = this->world->mining_config.find(chance_key);

		double chance = 50.0; // Default chance
		if (chance_setting != this->world->mining_config.end())
		{
			chance = static_cast<double>(chance_setting->second);
		}
		else
		{
			// Use default chance if item-specific not found
			auto default_chance = this->world->mining_config.find("DefaultChance");
			if (default_chance != this->world->mining_config.end())
			{
				chance = static_cast<double>(default_chance->second);
			}
		}

		// Roll for success
		if (util::rand(0.0, 100.0) <= chance)
		{
			// Success! Give the item and experience
			if (this->CanHoldItem(item_id, 1) > 0)
			{
				this->AddItem(item_id, 1);

				// Send packet to update client inventory (like quest giveitem does)
				if (item_id == 1) // Gold
				{
					PacketBuilder builder(PACKET_ITEM, PACKET_GET, 9);
					builder.AddShort(0); // UID
					builder.AddShort(item_id);
					builder.AddThree(1);
					builder.AddChar(static_cast<unsigned char>(this->weight));
					builder.AddChar(static_cast<unsigned char>(this->maxweight));
					this->Send(builder);
				...
				// Calculate and give experience
				int mining_exp_reward = CalculateMiningEXPReward(item_id);
				int regular_exp_reward = CalculateMiningRegularEXPReward(item_id);

				// Give mining EXP
				this->GiveMiningEXP(mining_exp_reward);

				// Give regular EXP if configured
				if (regular_exp_reward > 0)
				{
					bool level_up = false;
					this->exp += regular_exp_reward;
					this->exp = std::min(this->exp, int(this->world->config["MaxExp"]));

					while (this->level < int(this->world->config["MaxLevel"])
						&& this->exp >= this->world->exp_table[this->level+1])
					{
						this->level++;
						level_up = true;
						this->CalculateStats();
					...
					if (level_up)
					{
						...
						level_reply.AddInt(this->exp);
						level_reply.AddShort(this->level);
						level_reply.AddChar(this->statpoints);
						this->Send(level_reply);
					}
				}

				// Send mining announcement
				this->AnnounceMining(item_id, mining_exp_reward, regular_exp_reward);
	...
	return false; // No successful mining attempt
}

int Character::CalculateMiningEXPReward(int mined_item_id)
{
	// Check for item-specific mining EXP in mining.ini
	std::string item_key = util::to_string(mined_item_id) + ".mining_exp";
	auto mining_exp_setting = this->world->mining_config.find(item_key);

	if (mining_exp_setting != this->world->mining_config.end())
	{
		return static_cast<int>(mining_exp_setting->second);
	}

	// Use default mining EXP if item-specific setting not found
	auto default_setting = this->world->mining_config.find("DefaultMiningEXP");
	if (default_setting != this->world->mining_config.end())
	{
		return static_cast<int>(default_setting->second);
	}
...
int Character::CalculateMiningRegularEXPReward(int mined_item_id)
{
	// Check for item-specific regular EXP in mining.ini
	std::string item_key = util::to_string(mined_item_id) + ".regular_exp";
	auto regular_exp_setting = this->world->mining_config.find(item_key);

	if (regular_exp_setting != this->world->mining_config.end())
	{
		return static_cast<int>(regular_exp_setting->second);
	}

	// Use default regular EXP if item-specific setting not found
	auto default_setting = this->world->mining_config.find("DefaultRegularEXP");
	if (default_setting != this->world->mining_config.end())
	{
		return static_cast<int>(default_setting->second);
	}
...
std::string Character::ParseMiningMessage(const std::string& message, const std::string& item_name, int mining_exp, int regular_exp)
{
	std::string result = message;
	...
	pos = 0;
	while ((pos = result.find("{MINING_EXP}", pos)) != std::string::npos)
	{
		result.replace(pos, 12, util::to_string(mining_exp));
		pos += util::to_string(mining_exp).length();
	}
	...
	pos = 0;
	while ((pos = result.find("{MINING_LEVEL}", pos)) != std::string::npos)
	{
		result.replace(pos, 14, util::to_string(this->mining_level));
		pos += util::to_string(this->mining_level).length();
	}

	return result;
}

void Character::AnnounceMining(int mined_item_id, int mining_exp_reward, int regular_exp_reward)
{
	// Check if announcements are enabled
	auto announcements_enabled = this->world->mining_config.find("EnableAnnouncements");
	if (announcements_enabled != this->world->mining_config.end() && !static_cast<bool>(announcements_enabled->second))
	{
		return; // Announcements disabled
	}

	// Get item name
	std::string item_name = this->world->eif->Get(mined_item_id).name;

	// Check for item-specific announcement
	std::string announcement_template;
	std::string item_key = util::to_string(mined_item_id) + ".announcement";
	auto item_announcement = this->world->mining_config.find(item_key);

	if (item_announcement != this->world->mining_config.end())
	{
		announcement_template = static_cast<std::string>(item_announcement->second);
	}
	else
	{
		// Use default announcement format
		auto default_format = this->world->mining_config.find("AnnouncementFormat");
		if (default_format != this->world->mining_config.end())
		{
			announcement_template = static_cast<std::string>(default_format->second);
		}
		else
		{
			// Fallback to hardcoded message
			announcement_template = "[MINING] You have successfully mined {ITEM_NAME}! You received {MINING_EXP} EXP!";
		}
	}

	// Parse the message template with variables
	std::string announcement = ParseMiningMessage(announcement_template, item_name, mining_exp_reward, regular_exp_reward);

	this->StatusMsg(announcement);
}

bool Character::IsLorebookItem(int item_id)
{
	// Check if lorebooks are enabled
	auto lorebooks_enabled = this->world->lorebooks_config.find("EnableLorebooks");
	if (lorebooks_enabled != this->world->lorebooks_config.end() && !static_cast<bool>(lorebooks_enabled->second))
	{
		return false;
	}

	// Get the number of lorebooks configured
	auto lorebooks_amount = this->world->lorebooks_config.find("LorebooksAmount");
	if (lorebooks_amount == this->world->lorebooks_config.end())
	{
		return false;
	}

	int amount = static_cast<int>(lorebooks_amount->second);

	// Check each lorebook configuration
	for (int i = 1; i <= amount; ++i)
	{
		std::string item_key = util::to_string(i) + ".ItemID";
		auto item_config = this->world->lorebooks_config.find(item_key);

		if (item_config != this->world->lorebooks_config.end())
		{
			int config_item_id = static_cast<int>(item_config->second);
			if (config_item_id == item_id)
			{
				return true;
			}
		}
...
Lorebook Character::GetLorebook(int item_id)
{
	Lorebook lorebook;
	lorebook.item_id = item_id;
	...
	// Find the lorebook configuration for this item
	for (int i = 1; i <= amount; ++i)
	{
		std::string item_key = util::to_string(i) + ".ItemID";
		auto item_config = this->world->lorebooks_config.find(item_key);
		...
		if (item_config != this->world->lorebooks_config.end())
		{
			...
			if (config_item_id == item_id)
			{
				// Found the lorebook, get title and messages
				std::string title_key = util::to_string(i) + ".Title";
				auto title_config = this->world->lorebooks_config.find(title_key);
				if (title_config != this->world->lorebooks_config.end())
				{
					lorebook.title = static_cast<std::string>(title_config->second);
				}

				// Get all messages for this lorebook
				int page = 1;
				while (true)
				{
					std::string message_key = util::to_string(i) + ".Message[" + util::to_string(page) + "]";
					auto message_config = this->world->lorebooks_config.find(message_key);

					if (message_config != this->world->lorebooks_config.end())
					{
						std::string message = static_cast<std::string>(message_config->second);
						lorebook.messages.push_back(ParseLorebookMessage(message));
						page++;
					}
					else
					{
...
void Character::ShowLorebook(const Lorebook& lorebook)
{
	if (lorebook.messages.empty())
	{
		return;
	}

	// Create a proper dialog using the existing Dialog class
	Dialog dialog;

	// Add each message as a dialog page
	for (const auto& message : lorebook.messages)
	{
		dialog.AddPage(message);
	}

	// Create a simple quest dialog packet with the lorebook title
	PacketBuilder reply(PACKET_QUEST, PACKET_DIALOG, 10);
	reply.AddChar(1); // quest count (always 1 for lorebooks)
	reply.AddShort(0); // vendor ID (0 for lorebooks)
	reply.AddShort(0); // quest ID (0 for lorebooks)
	reply.AddShort(0); // session
	reply.AddShort(0); // dialog id
	reply.AddByte(255);

	// Add the lorebook title as the quest name
	reply.ReserveMore(3 + lorebook.title.length());
	reply.AddShort(0); // quest ID
	reply.AddBreakString(lorebook.title);

	// Build the dialog content using the Dialog class
	dialog.BuildPacket(reply);

	this->Send(reply);
}

std::string Character::ParseLorebookMessage(const std::string& message)
{
	std::string result = message;

	// Replace {PLAYER_NAME} with the character's name
	std::size_t pos = result.find("{PLAYER_NAME}");
	while (pos != std::string::npos)
	{
		result.replace(pos, 13, this->SourceName());
		pos = result.find("{PLAYER_NAME}", pos + this->SourceName().length());
	}
