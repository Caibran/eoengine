...
/**
 * Lorebook data structure for readable items
 */
struct Lorebook
{
	short item_id;
	...
	Lorebook() = default;
	Lorebook(short item_id, const std::string& title) : item_id(item_id), title(title) { }
};
...
class Character : public Command_Source
{
	public:
		...
		void GiveMiningEXP(int amount);
		bool IsMiningWeapon(int weapon_id);
		bool AttemptMining(unsigned char target_x, unsigned char target_y);
		int CalculateMiningEXPReward(int mined_item_id);
		int CalculateMiningRegularEXPReward(int mined_item_id);
		void AnnounceMining(int mined_item_id, int mining_exp_reward, int regular_exp_reward);
		std::string ParseMiningMessage(const std::string& message, const std::string& item_name, int mining_exp, int regular_exp);

		// Lorebook system methods
		bool IsLorebookItem(int item_id);
		Lorebook GetLorebook(int item_id);
		void ShowLorebook(const Lorebook& lorebook);
		std::string ParseLorebookMessage(const std::string& message);
