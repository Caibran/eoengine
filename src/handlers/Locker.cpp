...
void Locker_Add(Character *character, PacketReader &reader)
{
	...
	// Check if this is a bag operation
	// Use bag_open flag to determine if locker operations should go to bag
	if (character->bag_open && (x == 255 && y == 255 || character->map->GetSpec(x, y) != Map_Tile::BankVault))
	{
		// Bag operation
...
void Locker_Open(Character *character, PacketReader &reader)
{
	...
	if (util::path_length(character->x, character->y, x, y) <= 1)
	{
		if (character->map->GetSpec(x, y) == Map_Tile::BankVault)
		{
			// Player is opening a real bank locker, so close bag if it was open
			character->bag_open = false;
