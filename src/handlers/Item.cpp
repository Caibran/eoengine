...
void Item_Use(Character *character, PacketReader &reader)
{
	...
	// Check if this is a lorebook item
	if (character->IsLorebookItem(id))
	{
		// Show lorebook dialog without consuming the item
		Lorebook lorebook = character->GetLorebook(id);
		character->ShowLorebook(lorebook);
		return;
	}
