...
struct Map_Tile
{
	...
	bool Walkable(bool npc = false) const
	{
		...
		// DEBUG: Log when Unknown3 is being checked
		if (this->tilespec == Unknown3)
		{
			// Note: Can't use ServerMsg here as we don't have access to character
		...
		switch (this->tilespec)
		{
			...
			case Board8:
			case Jukebox:
			case Unknown3:  // Mining tiles should not be walkable
				return false;
			case NPCBoundary:
				return !npc;
			default:
